...
const columns = computed(() => {
    return [
        {
            title: t('account.header.fromAddress'),
            key: 'from',
            render: row => {
                return h(
                    'div',
                    {
                        ...
                        onClick: () => {
                            copy(row?.tx?.body?.messages?.[0]?.from_address)
                            message.success(t('wallet.copied'))
                        },
                    },
                    {
                        default: () => {
                            return encodeAddress(row?.tx?.body?.messages?.[0]?.from_address, false)
                        },
                    },
                )
            },
        ...
        {
            title: t('account.header.toAddress'),
            key: 'to',
            render: row => {
                return h(
                    'div',
                    {
                        class: 'hover:text-primary hover:cursor-pointer',
                        onClick: () => {
                            copy(row?.tx?.body?.messages?.[0]?.to_address)
                            message.success(t('wallet.copied'))
                        },
                    },
                    {
                        default: () => {
                            return encodeAddress(row?.tx?.body?.messages?.[0]?.to_address, false)
                        },
                    },
                )
            },
            ...
            render: row => {
                return h('div', {}, [
                    h(
                        ...
                        {
                            default: () => getActualAmount(row?.tx?.body?.messages?.[0]?.amount?.[0]?.amount),
                        },
                    ),
                    h(
                        'span',
                        {},
                        {
                            default: () => getTokenDisplayName(row?.tx?.body?.messages?.[0]?.amount?.[0]?.denom),
                        },
                    ),
                ])
            },
