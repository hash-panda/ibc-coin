...
watch(
    ...
    (newValue, oldValue) => {
        sectionName.value =
            newValue < 10000 ? (newValue < 1000 ? (newValue < 100 ? (newValue < 25 ? 'FREE' : 'INTERN') : 'ENGINEER') : 'ASTRONAUT') : 'MOONWALKER'
    },
    { immediate: true },
)
</script>
<template>
    <div>
        <div class="w-full flex justify-center mt-8 md:mt-16">
            <div class="w-3/4 md:w-1/2">
                <article class="prose dark:prose-invert text-center">
                    ...
                    <p>
                        Enjoy premium features on top of ATOM staking
                        <br />
                        rewards (~10% APR) by staking with our validator
                    </p>
                    <p></p>
                    <!-- ... -->
                </article>
                <div class="relative">
                    <div
                        ...
                        :style="`left: calc(${stakeValue <= 1000 ? stakeValue / 10 : 100}% - 3rem);`"
                    >
                        {{ sectionName }}
                    </div>
                    <input type="range" min="0" max="1000" :value="stakeValue" class="range range-primary mt-10" @input="slideChange" />
                </div>
                <div class="mt-4">
        ...
        <div class="grid mx-10 lg:mx-48 my-10">
            <div class="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5 px-2 lg:px-0 text-gray-100">
                <!-- ring-2 ring-violet-500 scale-[1.025] -->
                <div
                    v-for="config in priceConfig"
                    :key="config.id"
                    :class="config.title ? (sectionName === config.title ? cardClass + ' active' : cardClass) : ''"
                >
                    <div v-if="config.title">
                        <div class="py-6 sm:py-10 flex justify-center items-center text-xl uppercase tracking-widest font-bold">
                            {{ config.title }}
                        </div>
